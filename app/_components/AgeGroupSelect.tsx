 ...
 */
export const getAgeGroupFromAge = (age: number | undefined): string => {
  if (age === undefined) {
    return '';
  }
...
interface AgeGroupSelectProps {
  age?: number;
  selectedAgeGroup: (value: string) => void;
}

function AgeGroupSelect({ age, selectedAgeGroup }: AgeGroupSelectProps) {
  const [ageGroups, setAgeGroups] = useState<AgeGroup[]>([]);
  const [selectedGroup, setSelectedGroup] = useState<string>('2-5yrs');
