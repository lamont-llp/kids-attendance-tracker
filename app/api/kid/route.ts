...
export async function PUT(req: NextRequest) {
    try {
        ...
        if (!data?.name || !data?.age) {
            return NextResponse.json({ message: 'Missing required fields (name and age)' }, { status: 400 });
        }
        ...
        return NextResponse.json({ message: 'Kid updated successfully', id: parseInt(id) }, { status: 200 });
    } catch (error) {
